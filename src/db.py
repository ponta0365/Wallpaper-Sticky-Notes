...
def init_db():
    ...
    try:
        cursor.execute("SELECT reminder_status FROM memos LIMIT 1")
    except sqlite3.OperationalError:
        try:
            cursor.execute("ALTER TABLE memos ADD COLUMN reminder_status TEXT NOT NULL DEFAULT 'pending'")
            cursor.execute("ALTER TABLE memos ADD COLUMN reminded_at TEXT")
        except Exception as e:
            print("Migration warning (reminder status fields):", e)
...
def add_memo(body, color="#FFFFC8", monitor_id="0", x=None, y=None, width=200, height=200, reminder_at=None, font_size=12, reminder_status='pending', reminded_at=None):
    """新規メモを追加します。"""
    ...
    cursor.execute("""
    INSERT INTO memos (body, color, monitor_id, x, y, width, height, reminder_at, font_size, reminder_status, reminded_at, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (body, color, str(monitor_id), x, y, width, height, reminder_at, font_size, reminder_status, reminded_at, now, now))
...
def export_data(file_path):
    ...
    cursor.execute("SELECT body, color, monitor_id, x, y, width, height, reminder_at, font_size, reminder_status, reminded_at FROM memos WHERE status = 'active'")
    memos = [dict(row) for row in cursor.fetchall()]
    ...
    try:
        ...
        for m in memos:
            ...
            """, (
                ...
                m.get("reminder_at"), m.get("font_size", 12),
                m.get("reminder_status", "pending"), m.get("reminded_at"),
                now, now
            ))
